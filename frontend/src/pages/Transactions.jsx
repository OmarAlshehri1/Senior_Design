...
export default function Transactions() {
  ...
  const filtered = useMemo(() => {
    let list = sortNewestFirst(transactions.filter((t) => !t.processing));
    ...
    if (riskFilter !== 'All') {
      list = list.filter((t) => getRiskLevel(t.riskScore) === `${riskFilter} Risk`);
    }
  ...
  return (
    <>
      ...
      <div className="toolbar">
        ...
        <div className="filter-chip-group" role="group" aria-label="Filter by risk level">
          {riskFilters.map((f) => (
            <button
              type="button"
              key={f}
